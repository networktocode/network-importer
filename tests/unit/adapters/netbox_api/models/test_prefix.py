...
def test_translate_attrs_for_netbox_default(netbox_api_base):
    ...
    prefix = NetboxPrefix(dsync=netbox_api_base, prefix="10.1.111.0/24", site_name="HQ", remote_id=44)
...
def test_update_prefix(requests_mock, netbox_api_base):
    ...
    prefix = NetboxPrefix(dsync=netbox_api_base, prefix="10.1.111.0/24", site_name="HQ", remote_id=remote_id)
...
def test_create_prefix_with_vlan(requests_mock, netbox_api_base):
    ...
    prefix = NetboxPrefix.create(
        dsync=netbox_api_base, ids=dict(prefix="10.1.111.0/24", site_name="HQ"), attrs=dict(vlan="HQ__111")
    )
